...
public class ListDetailView extends JFrame {
    ...
    public ListDetailView(User currentUser, PackingList list) {
        ...
        initComponents();
        loadItemsAndProgressInBackground();
    }
    ...
    private void initComponents() {
        ...
        searchField.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                filterItemsInBackground(searchField.getText());
            }
        });
    ...
    private JPanel createHeader() {
        ...
        deleteButton.addActionListener(e -> deleteListInBackground());
    ...
    private JPanel createAddButtonPanel() {
        ...
        addButton.addActionListener(e -> addNewItemInBackground());
    ...
    private void loadItemsAndProgressInBackground() {
        SwingWorker<List<Item>, Void> worker = new SwingWorker<>() {
            ...
            protected List<Item> doInBackground() {
                return packingListDAO.getItemsByListId(currentList.getListId());
            }
    ...
    private void filterItemsInBackground(String searchText) {
        SwingWorker<List<Item>, Void> worker = new SwingWorker<>() {
            @Override
            protected List<Item> doInBackground() {
                return packingListDAO.getItemsByListId(currentList.getListId());
    ...
    private void addNewItemInBackground() {
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            SwingWorker<Boolean, Void> worker = new SwingWorker<>() {
                ...
                protected Boolean doInBackground() {
                    ...
                    return packingListDAO.addItemToList(newItem);
                }
                ...
                protected void done() {
                    try {
                        boolean success = get();
                        if (success) loadItemsAndProgressInBackground();
                        else JOptionPane.showMessageDialog(ListDetailView.this, "Failed to add item", "Error", JOptionPane.ERROR_MESSAGE);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                }
    ...
    private void deleteItemInBackground(Item item) {
        SwingWorker<Boolean, Void> worker = new SwingWorker<>() {
            @Override
            protected Boolean doInBackground() {
                return packingListDAO.deleteItem(item.getItemId());
            }

            @Override
            protected void done() {
                try {
                    if (get()) loadItemsAndProgressInBackground();
                    else JOptionPane.showMessageDialog(ListDetailView.this, "Failed to delete item", "Error", JOptionPane.ERROR_MESSAGE);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
    ...
    private void deleteListInBackground() {
        ...
        SwingWorker<Boolean, Void> worker = new SwingWorker<>() {
            @Override
            protected Boolean doInBackground() {
                return packingListDAO.deletePackingList(currentList.getListId());
            }
            ...
            protected void done() {
                try {
                    if (get()) {
                        JOptionPane.showMessageDialog(ListDetailView.this, "Packing list deleted successfully");
                        dispose();
                    } else {
                        JOptionPane.showMessageDialog(ListDetailView.this, "Failed to delete packing list", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
    ...
    private void updateProgressBarInBackground() {
        SwingWorker<Void, Void> worker = new SwingWorker<>() {
            int total, packed;

            @Override
            protected Void doInBackground() {
                total = packingListDAO.getTotalItemsCount(currentList.getListId());
                packed = packingListDAO.getPackedItemsCount(currentList.getListId());
                return null;
            }
            ...
            protected void done() {
                ...
                int percent = (total > 0) ? (int) ((packed * 100.0) / total) : 0;
    ...
    private JPanel createItemCard(Item item) {
        ...
        checkBox.addItemListener(e -> {
            ...
            new Thread(() -> packingListDAO.setItemPackedStatus(item.getItemId(), checkBox.isSelected())).start();
            updateProgressBarInBackground();
        });

        JLabel nameLabel = new JLabel(item.getItemName());
        nameLabel.setFont(new Font("Arial", Font.BOLD, 14));
        if (item.isPacked()) nameLabel.setForeground(Color.GRAY);
        ...
        deleteButton.setFocusPainted(false);
        deleteButton.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        deleteButton.addActionListener(e -> deleteItemInBackground(item));
